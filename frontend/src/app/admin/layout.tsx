...
const navItems = [
  ...
  { href: "/admin#fl-rounds", icon: Network, label: "FL Rounds" },
  { href: "/admin#credits", icon: Zap, label: "Credits" },
];
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user || user.role !== "GLOBAL_ADMIN") return null;
    ...
    <div className="min-h-screen bg-slate-950 bg-grid flex">
      ...
      <div className="lg:hidden fixed top-0 inset-x-0 h-14 glass z-40 flex items-center justify-between px-4 border-b border-slate-800/50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-heading font-bold text-sm text-white">MedChain-FL</span>
        </div>
        <button
          onClick={() => { logout(); router.push("/login"); }}
